...
**M1–M3 已完成**:核心純函式狀態機 + LangGraph adapter 持久化(kill 後同 thread_id 續跑)+ 分層驗收閘門(hard→soft→human)與完整 HITL(approved/rejected/edited),全 mock、不需 LLM key。
...
真 LLM planner/judge(M4,soft 層接真 Claude)、審計輸出(M5)尚未實作,規格見 [`docs/phase2/`](docs/phase2/)。
...
```bash
.venv/bin/pytest -q               # 全套:engine T1–T8 + adapter A1–A12 + verifier V1–V11 + D9 import 邊界
