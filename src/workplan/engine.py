...
def _require_running(state: PlanState) -> Step:
    ...
    if state.status == "blocked":
        raise IllegalTransitionError("state 為 blocked,僅允許 on_human_resolved")
    if state.status != "running":
        raise IllegalTransitionError(f"state.status={state.status!r},不接受新的轉移")
...
def on_executed(
    ...
    events = [
        _emit(
            ...
            step_id=step.id,
            content=_summarize(output.content),
            error=output.error,
        )
    ]
...
def _handle_failure(
    ...
    if state.replans < max_replans:  # R:重規劃
        ...
        brief = (
            f"步驟 {step.id} 已嘗試 {step.attempts} 次仍未通過驗收。最後回饋:{feedback}"
        )
        return Decision(Action.REPLAN, state, events, feedback=brief)
    # 兩者皆用盡 → D8 escalate(非直接 FAILED)
    return _escalate(state, "retries+replans exhausted", exhausted=True, events=events)
...
def _escalate(
    ...
    state.pending_human = HumanGate(step_id=step.id, reason=reason, asked_at=_now())
    events.append(
        _emit(
            state,
            EventType.ESCALATED,
...
def on_replanned(state: PlanState, new_plan: Plan) -> Decision:
    ...
    events = [
        _emit(
            ...
            to_version=state.plan.version,
            reason=new_plan.revision_note,
            kept_step_ids=[s.id for s in kept],
        ),
        _start_current_step(state),
    ]
    return Decision(Action.EXECUTE, state, events)
